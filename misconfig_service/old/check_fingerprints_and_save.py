...
def create_output_folder() -> str:
    ...
    if not os.path.exists(folder_name):
        ...
        print(f"DEBUG: Created output folder: {folder_name}")
...
def save_response_data(response_data: Dict[str, Any], output_folder: str) -> str:
    """Save response content and headers to files"""
    ...
    url_folder = os.path.join(output_folder, safe_filename)
    if not os.path.exists(url_folder):
        os.makedirs(url_folder)
    
    # Save HTML content
    content_file = os.path.join(url_folder, "content.html")
    try:
        with open(content_file, "w", encoding="utf-8") as f:
            f.write(response_data["content"])
        print(f"DEBUG: Saved content to {content_file}")
    except Exception as e:
        print(f"DEBUG: Error saving content: {e}")
    
    # Save headers as JSON
    headers_file = os.path.join(url_folder, "headers.json")
    try:
        headers_data = {
            "url": url,
        ...
        print(f"DEBUG: Saved headers to {headers_file}")
    except Exception as e:
        print(f"DEBUG: Error saving headers: {e}")
    
    # Save metadata
    metadata_file = os.path.join(url_folder, "metadata.txt")
    try:
        with open(metadata_file, "w", encoding="utf-8") as f:
            f.write(f"URL: {url}\n")
            f.write(f"Status Code: {response_data['status_code']}\n")
            f.write(f"Content Length: {len(response_data['content'])} characters\n")
            f.write(f"Headers Count: {len(response_data['headers'])}\n")
...
def scan_target(services: List[Dict[str, Any]], target_data: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    print(f"DEBUG: Results will be saved to: {output_folder}")
    ...
    # Save scan summary
    save_scan_summary(all_matches, output_folder, target_data)
    
    print(f"\nDEBUG: === SCAN COMPLETE ===")
    print(f"DEBUG: Total fingerprint matches found: {len(all_matches)}")
    print(f"DEBUG: All data saved to: {output_folder}")
    return all_matches

def save_scan_summary(matches: List[Dict[str, Any]], output_folder: str, target_data: Dict[str, Any]) -> None:
    """Save scan summary to JSON file"""
    summary_file = os.path.join(output_folder, "scan_summary.json")
    
    summary = {
        "scan_timestamp": datetime.now().isoformat(),
        "total_matches": len(matches),
        "target_info": {
            "total_urls_scanned": len(extract_urls_from_target(target_data)),
            "web_technologies": len(target_data.get("web_technologies", {})),
            "directories": len(target_data.get("directories", [])),
            "api_endpoints": len(target_data.get("api_endpoints", []))
        },
        "matches": matches
    }
    ...
    try:
        ...
        print(f"DEBUG: Saved scan summary to {summary_file}")
    except Exception as e:
        print(f"DEBUG: Error saving scan summary: {e}")
    ...
    try:
        ...
        if matches:
            ...
            for i, match in enumerate(matches, 1):
                ...
                # Show saved folder location
                if match.get('saved_folder'):
                    print(f"Data saved to: {match['saved_folder']}")
